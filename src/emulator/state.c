...
void run(State* s) {
    ...
    while (run) {
        ...
        /*if (push1 == 119386 && push2 == 0) {    // push (stmdb)
              ...
              uint32_t rn = extract(inst,12,15);
              for(int z = 16 ; z <= 31 ; z++){
                  setBits[31-z] = extract(inst,z,z);
                  if(extract(inst,z,z))
                      numReg++;
              }
              if(!extract(inst,11,11)){//stm
                  start_address = s->gprs[rn] - (numReg * 4);
                  //end_address = s->gprs[rn] - 4;
                  if(conditionPassed(s,cond)){
                      if(extract(inst,10,10))
                          s->gprs[rn] = s->gprs[rn] - (numReg * 4);
                      uint32_t address = start_address;
                      for (int i = 0 ; i < 16 ; i++){
                          if(setBits[i]){
                              write32(s->mem,address,s->gprs[i]);
                              address+=4;
                          }
                      }
                  ...
                  start_address = s->gprs[rn];
                  //end_address = s->gprs[rn] + (numReg * 4) - 4;
                  if(conditionPassed(s,cond)){
                      if(extract(inst,10,10))
                          s->gprs[rn] = s->gprs[rn] + (numReg*4);
                      uint32_t address = start_address;
